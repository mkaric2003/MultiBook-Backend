...
func (r *Queries) availableSlots(ctx context.Context, businessID, staffID uuid.UUID, input domain.AvailabilityInput) (domain.AvailableSlots, error) {
	...
	rows, err := r.pool.Query(ctx, `
		...
		WHERE minute + $3 <= 1440
		  ...
		  AND NOT EXISTS(SELECT 1 FROM service_appointments a WHERE a.staff_id=$1 AND a.status='confirmed' AND ($5::uuid IS NULL OR a.id <> $5) AND a.scheduled_range && tstzrange($4 + minute * interval '1 minute', $4 + (minute+$3) * interval '1 minute','[)'))
		ORDER BY minute`, staffID, weekday, duration, localStart, input.ExcludeAppointmentID)
	if err != nil {
