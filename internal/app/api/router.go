...
func NewRouter(appContext context.Context, pool *pgxpool.Pool, authenticator *auth.FirebaseAuthenticator, cfg config.Config, logger *slog.Logger) http.Handler {
	...
	appointmentsHandler := appointmentshttp.NewHandler(appointmentsapp.NewService(appointmentspostgres.NewCommandRepository(pool, auditWriter), appointmentspostgres.NewQueries(pool), notificationsService))
	bookingsHandler := bookingshttp.NewHandler(bookingsapp.NewService(bookingspostgres.NewRepository(pool), notificationsService))
